...
int redirect_fds(const char *stdin_file, const char *stdout_file, const char *stderr_file)
{
    ...
    if (stdin_file) {
        ret = redirect_fd(STDIN_FILENO, stdin_file, O_RDONLY);
        if (ret)
            return ret;
    }

    if (stdout_file) {
        ret = redirect_fd(STDOUT_FILENO, stdout_file, O_WRONLY | O_CREAT | O_TRUNC);
        if (ret)
            return ret;
    }
    ...
    if (stderr_file) {
        ...
        ret = redirect_fd(STDERR_FILENO, stderr_file, O_WRONLY | O_CREAT | O_APPEND);
        if (ret)
            return ret;
    }
                         ...
                         const char *stderr_file)
{
    ...
    ret = redirect_fds(stdin_file, stdout_file, stderr_file);
    if (ret)
        goto fail;
 ...
 *
 * @param command short human-readable description of the command (perhaps argv[0])
 *
